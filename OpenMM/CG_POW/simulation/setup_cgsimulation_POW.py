...
parser.add_argument("--pdb", default="POG5POWPOG6_1trimervs3.pdb")
parser.add_argument("--outprefix", default="POG5POWPOG6_1trimervs3_300K")
...
for atom in topology.atoms():
    name = atom.name
    mass = {"BBP": 3.0, "BBO": 3.0, "BBG": 3.0, "BBW": 3.0, "HBP": 1.0, "HBG": 1.0, "HBW": 1.0}.get(name, 1.0)
...
angle_params = [
    ...
    ("BBP", "BBO", "BBW", 3.14159, 2),
    ("BBO", "BBW", "BBP", 3.14159, 2),
    ("BBW", "BBP", "BBO", 3.14159, 2),
    ("HBP", "BBP", "BBO", 1.5708, 30),
    ("HBG", "BBG", "BBO", 1.5708, 30),
    ("HBW", "BBW", "BBO", 1.5708, 30),
    ("HBW", "BBW", "BBP", 1.5708, 30)
]

torsion_params = [
    ("HBP", "BBP", "BBG", "HBG", 1, 2.0944, 30),
    ("HBG", "BBG", "BBP", "HBP", 1, -2.0944, 30),
    ("HBP", "BBP", "BBW", "HBW", 1, 2.0944, 30),
    ("HBW", "BBW", "BBP", "HBP", 1, -2.0944, 30)
]
...
for chain in topology.chains():
    ...
    for i, res in enumerate(residues):
        ...
        # Inter-residue bond (connect BBW[i] → BBP[i+1])
        if i < len(residues) - 1:
            this_atoms = {a.name: a.index for a in residues[i].atoms()}
            next_atoms = {a.name: a.index for a in residues[i+1].atoms()}
            if "BBW" in this_atoms and "BBP" in next_atoms:
                bond_force.addBond(this_atoms["BBW"], next_atoms["BBP"],
                                   0.25 * unit.nanometer, 100 * unit.kilocalorie_per_mole / unit.nanometer**2)
...
type_map = {"BBP":0, "BBO":1, "BBG":2, "HBG":3, "HBP":4, "HBW":5, "BBW":6}
M = 7
...
epsilon_bbhb[5,2] = 1.0
sigma_bbhb[3,2]   = 0.65
sigma_bbhb[4,2]   = 0.65
sigma_bbhb[5,2]   = 0.8

epsilon_bbhb[6,3] = 1.0    # BB–HB
epsilon_bbhb[6,4] = 1.0
epsilon_bbhb[6,5] = 1.0
sigma_bbhb[6,3]   = 0.65
sigma_bbhb[6,4]   = 0.65
sigma_bbhb[6,5]   = 0.8

epsilon_bbhb[3,6] = 1.0    # BB–HB
epsilon_bbhb[4,6] = 1.0
epsilon_bbhb[5,6] = 1.0
sigma_bbhb[3,6]   = 0.65
sigma_bbhb[4,6]   = 0.65
sigma_bbhb[5,6]   = 0.8
...
epsilon_hbda[3,4] = 242.3
sigma_hbda[3,4]   = 0.3
epsilon_hbda[4,3] = 242.3
sigma_hbda[4,3]   = 0.3
